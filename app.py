...
if __name__ == '__main__':
    ...
    DAOCLOUD_APPNAME = os.getenv('DAOCLOUD_APP_NAME', "x")
    DAOCLOUD_APP_RELEASE = os.getenv('DAOCLOUD_APP_RELEASE', "x")

    start_time = time.time()
    daoapi = dao_api(DAOCLOUD_APITOKEN)
    app = daoapi.app_by_name(DAOCLOUD_APPNAME)
    print("find app %s", app)
    app_redeploy_result = daoapi.app_redeploy(daoapi.app_by_name(DAOCLOUD_APPNAME)['app'][0]['id'],
                                              DAOCLOUD_APP_RELEASE)
    print("app_redeploy result:", app_redeploy_result)
    if 'action_id' not in app_redeploy_result:
        exit(1)
    for i in range(1, 15):
        result = daoapi.actions(daoapi.app_by_name(DAOCLOUD_APPNAME)['app'][0]['id'], app_redeploy_result['action_id'])
